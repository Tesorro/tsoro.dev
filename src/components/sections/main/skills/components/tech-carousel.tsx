...
export function TechCarousel() {
	...
	return (
		<motion.div
			...
			variants={fromBottom}
			className='relative mb-4 rounded-2xl bg-transparent [mask-image:linear-gradient(to_right,transparent,black_8%,black_92%,transparent)] px-2 py-3'
			...
			>
				<CarouselContent className='-ml-2'>
					{longIcons.map((icon, i) => (
						...
						>
							<div
								className='flex h-20 items-center justify-center rounded-xl px-3 backdrop-blur-[2px] transition-transform duration-300 hover:scale-[1.05]'
								title={icon.alt}
								aria-label={icon.alt}
							>
								<Image
